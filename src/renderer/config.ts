...
export function renderConfigPage(
    ...
    (window as any).addEmailFilter = addEmailFilter;
    (window as any).addBulkEmails = addBulkEmails;
    (window as any).removeEmailFilter = removeEmailFilter;
    (window as any).toggleEmailFilter = toggleEmailFilter;
    (window as any).addRutFilter = addRutFilter;
    (window as any).addBulkRuts = addBulkRuts;
    (window as any).removeRutFilter = removeRutFilter;
    (window as any).toggleRutFilter = toggleRutFilter;
    (window as any).showEliminatedReport = showEliminatedReport;
    (window as any).exportEliminatedReport = exportEliminatedReport;
...
function showConfirmModal(message: string, onConfirm: () => void, onCancel?: () => void): void {
    // Crear el modal
...
function addBulkEmails() {
    const textarea = document.getElementById('bulkEmailInput') as HTMLTextAreaElement;
...
function addRutFilter() {
    const input = document.getElementById('rutInput') as HTMLInputElement;
...
function addBulkRuts() {
    const textarea = document.getElementById('bulkRutInput') as HTMLTextAreaElement;
...
function updateRutFiltersUI() {
    ...
    // Asegurar que el input esté habilitado después de actualizar la UI
    const rutInput = document.getElementById('rutInput') as HTMLInputElement;
    if (rutInput) {
        rutInput.disabled = false;
    }
}
...
function updateUI() {
    ...
    const maxFilesInput = document.getElementById('maxFiles') as HTMLInputElement;
    if (maxFilesInput) {
